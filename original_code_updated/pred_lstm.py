...
class AWLSTM:
    def __init__(self, data_path, model_path, model_save_path, parameters, steps=1, epochs=50,
        ...
        print(tf.shape(self.tra_pv))
        # self.tra_pv, self.tra_wd, self.tra_gt, \
        # self.val_pv, self.val_wd, self.val_gt, \
        # self.tes_pv, self.tes_wd, self.tes_gt = None
    ...
    def construct_graph(self):
        ...
        with tf.device(device_name):
            ...
            self.lstm_cell = tf.compat.v1.nn.rnn_cell.BasicLSTMCell(
                self.paras['unit']
            )
...
if __name__ == '__main__':
    ...
    parser.add_argument('-la', '--beta_adv', type=float, default=5e-2,
                        help='beta for adverarial loss')
    parser.add_argument('-le', '--epsilon_adv', type=float, default=1e-3,
